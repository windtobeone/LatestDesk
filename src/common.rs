...
pub fn load_custom_client() {
    ...
    const COMPILE_TIME_CONFIG: &str = include_str!("custom.txt");
    if !COMPILE_TIME_CONFIG.trim().is_empty() && !COMPILE_TIME_CONFIG.contains("placeholder") {
        read_custom_client(COMPILE_TIME_CONFIG.trim());
    }
}

  ...
  if (displayWidth !== lastWidth || displayHeight !== lastHeight) {
    ...
    if (YUVCanvas.WebGLFrameSink.isAvailable()) {
      try {
        ...
        // WebGL Context Lost Guard: prevent fatal crash when >16 WebGL contexts exist
        canvas.addEventListener('webglcontextlost', (e) => {
          e.preventDefault();
          console.warn("[globals.js Guard] WebGL Context lost. Gracefully falling back to 2D canvas.");
          try {
            yuvCanvas = YUVCanvas.attach(canvas, { webGL: false });
          } catch (err) {
            console.error("[globals.js Guard] Fallback to 2D canvas failed:", err);
          }
        }, false);
        yuvCanvas = YUVCanvas.attach(canvas, { webGL: true });
        gl = canvas.getContext("webgl", { preserveDrawingBuffer: true }) || canvas.getContext("experimental-webgl", { preserveDrawingBuffer: true });
        if (window.isMonitor === 'Y' && typeof window.initMonitorGpuRenderer === 'function') {
          try {
            gpuRenderer = window.initMonitorGpuRenderer(canvas);
            window.gpuRenderer = gpuRenderer;
          } catch (mErr) {
            console.error("[globals.js] Monitor GPU Renderer hook error:", mErr);
          }
        }
...
export function wipeMemory() {
  console.log("[globals.js Guard] Physical Memory Wipe: Overwriting VRAM textures with 1x1 black pixels...");
  try {
    if (gpuRenderer) {
      gpuRenderer.destroyGpuResources(true);
    }
    if (gl) {
    ...
    console.error("[globals.js Guard] Memory wipe error:", e);
  }
  if (pixels) {
    pixels.fill(0);
  }
